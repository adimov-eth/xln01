...
const power = (sigs: Map<Address, string>) => sigs.size; // in our genesis, each signer has 1 share
...
export function applyServerBlock({ prev, batch, timestamp }: ApplyServerBlockParams): ApplyServerBlockResult {
	...
	const { finalReplicas, allOutbox } = batch.reduce<{ finalReplicas: Map<string, Replica>; allOutbox: Input[] }>(
		(acc, input) => {
			const { cmd } = input;
				...
				switch (cmd.type) {
						...
						if (!replica.proposal && updatedReplica.proposal) {
							...
							return Object.keys(updatedReplica.last.state.quorum.members).map(s => ({
								...
								cmd: {
									...
									addrKey: cmd.addrKey,
									signer: s as Address,
									frameHash: updatedReplica.proposal?.hash || (DUMMY_SIGNATURE as Hex),
					...
					case 'SIGN': {
						if (updatedReplica.isAwaitingSignatures && updatedReplica.proposal) {
							...
							const prevPower = replica.proposal ? power(replica.proposal.sigs) : 0;
							const newPower = power(proposal.sigs);
