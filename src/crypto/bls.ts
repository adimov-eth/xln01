...
export const verify = async (msg: Uint8Array, sig: Hex, pb: PubKey): Promise<boolean> => bls.verify(hexToBytes(sig), msg, pb);
...
export const verifyAggregate = (hanko: Hex, msgHash: Hex, pubs: PubKey[]): boolean =>
  bls.verifyBatch(hexToBytes(hanko), pubs.map(() => hexToBytes(msgHash)), pubs);
